        ...
        while 1:
                            ...
                            if 1200 > cursor.rect.x > 954 and \
                               900 > cursor.rect.y > 809:
                                ...
                                if numBlocks > 0:
                                    ...
                                    timeText.change(self.strings['atLeastOne'])
                        #main menu button
                        if 240 > cursor.rect.x > 0 and \
                           900 > cursor.rect.y > 809:
                            self.mainMenu = MainMenu()

            #for block in blocks:
            self.x = cursor.rect.x
            self.y = cursor.rect.y
            ...
            if not editMode and not completed:
                for block in allBlocks:
                    ...
                    if (curMode == 1 and not (dif == 1 or dif == 2)) or \
                       curMode == 2 or (curMode == 3 and not dif == 1):
                        if block.isLast == 1 and \
                           (keystate[K_KP1] or keystate[K_RETURN]) and \
                           not keyDown:
                            block.rotate()
                            numRotate += 1
                            keyDown = True
                    ...
                    if event.get_grab():
                        ...
                        if block.rect.collidepoint([cursor.rect.x,
                                                    cursor.rect.y]):
                            anotherBlock = 0
                            hi.setpos([block.rect.x, block.rect.y])
                            for blockB in allBlocks:
                                if blockB.isMoving and blockB != block:
                                    anotherBlock = 1
                                    break
                            if anotherBlock == 0:
                                if curMode == 1 or curMode == 3:
                                ...
                                isRan = 1
                                break
                        elif block.isMoving and (curMode == 1 or curMode == 3):
                            block.grab([cursor.rect.x, cursor.rect.y])
                    #snapping to grid
                    elif isRan == 1 and block.isLast == 1:
                        for piece in gridpos:
                            if piece[0] + 108 > cursor.rect.x > piece[0] and \
                               piece[1] + 108 > cursor.rect.y > piece[1] and \
                               block.isMoving and \
                               (curMode == 1 or curMode == 3):
                                place = piece[0] + 54, piece[1] + 54
                                isLast.grab(place)
                                isRan = 0
                        block.setGrabbed(False)
                    if block.rect.collidepoint(
                        [cursor.rect.x, cursor.rect.y]) and \
                            event.get_grab():
                        hi.setpos([block.rect.x, block.rect.y])
                    """else:
hi.setpos([block.rect.x,block.rect.y])
continue"""
                    #else:
                    # hi.setpos([1300,900])
                    #elif not (cursor.rect.x > block.rect.x
                    #and cursor.rect.x < block.rect.x +96 and
                    #cursor.rect.y > block.rect.y and
                    #cursor.rect.y < block.rect.y + 96):
                    # hi.setpos([1300,900])
            #board editor
            elif editMode:
                if event.get_grab():
                    for block in allBlocks:
                        if block.rect.collidepoint([cursor.rect.x,
                            ...
                            n = ask(self.screen,
                                    "Enter North",
                                    529, 30, 300, 30, 42, True, 6)
                            east = ask(self.screen,
                                       "Enter East",
                                       529, 30, 300, 30, 42, True, 6)
                            s = ask(self.screen,
                                    "Enter South",
            ...
            if completed and nname == 'null' and not failed:
                ...
                nname = ask(self.screen,
                            "Name", 400, 382, 300, 30, 42, False, 8)

                if not curMode == 2:
                    #rewrites the highscores so most recent is at top,
                    #and only 5 total are ever written
                    ...
                    #if against time, calculate time taken,
                    #and add leading zeroes if necessary
                    else:
                        mins = origMin - mins
                    ...
                    if curMode == 1:
                        scores = open(
                            "scores" + os.sep + "timeRecent.txt", 'w')
                    elif curMode == 3:
                        scores = open(
                            "scores" + os.sep + name + "Scores.txt", 'w')
...
"""
        lines = chain(*(pygame.wrapline(
            line, font, maxwidth) for line in text.splitlines()))
        return list(lines)
    ...
    def loop(self):
        ...
        while 1:
            ...
            for pyevent in pygame.event.get():
                if pyevent.type == QUIT or \
                   (pyevent.type == KEYDOWN and pyevent.key == K_ESCAPE):
                        pygame.quit()
                ...
                if hit:
                    ...
                    if 777 > cursor.rect.x > 466 and \
                       451 > cursor.rect.y > 329 and \
                       not self.instBool and not self.modeBool:
                        pygame.display.flip()
                        game = Game()
                        game.run(self.curSel,
                                 self.selRange,
                                 self.selDiff,
                                 self.selSubtraction,
                                 self.text,
                    ...
                    if 865 > cursor.rect.x > 798 and \
                       599 > cursor.rect.y > 573 and \
                       self.instBool:
                        self.menuImg = load_image('bg.png')
                        self.screen.blit(self.menuImg, (0, 0))
                        self.instBool = False
                    ...
                    if self.modeBool and not self.goodLoad:
                        #which is selected?
                        if self.curSel == 1:
                        ...
                        if 435 > cursor.rect.x > 109 and \
                           354 > cursor.rect.y > 176:
                            self.curSel = 1
                            self.menuImg.blit(self.gModes, (101, 167))
                            #which is selected?
                            self.menuImg.blit(self.hi, (127, 220))
                        ...
                        if self.curSel == 1 or self.curSel == 2:
                            difficulty = pygame.font.Font(None, 42).render(
                                self.strings['selectDifficulty'], 1, (0, 0, 0))
                            ...
                            if self.curSel == 1:
                                numRange = pygame.font.Font(None, 42).render(
                                    self.strings['selectNumber'], 1, (0, 0, 0))
                                ...
                                #range left arrow
                                if 554 > cursor.rect.x > 529 and \
                                   433 > cursor.rect.y > 391:
                                    self.rangeChange(1)

                                #range right arrow
                                if 934 > cursor.rect.x > 909 and \
                                   433 > cursor.rect.y > 39:
                                    self.rangeChange(2)

                                #casual left arrow
                                if 554 > cursor.rect.x > 529 and \
                                   296 > cursor.rect.y > 254:
                                    self.casChange()
                        ...
                        if self.curSel == 3:
                            ...
                            boardEnter = pygame.font.Font(None, 42).render(
                                self.strings['enterBoardName'], 1, (0, 0, 0))
                            self.menuImg.blit(boardEnter, (526, 350))

                            createText = pygame.font.Font(None, 42).render(
                                self.strings['createBoard'], 1, (0, 0, 0))
                            self.menuImg.blit(createText, (570, 225))

                            #delete button
                            delText = pygame.font.Font(None, 40).render(
                                self.strings['delete'], 1, (0, 0, 0))
                            ...
                            #delete button
                            if 1095 > cursor.rect.x > 981 and \
                               418 > cursor.rect.y > 371:
                                self.tempList = self.boardList
                                toDeleteFile = file('data' +
                                                    os.sep +
                                                    'boardList.txt', 'w')
                            ...
                            if 806 > cursor.rect.x > 546 and \
                               265 > cursor.rect.y > 215:
                                game = Game()
                                game.run(self.curSel,
                                         self.selRange,
                                         self.selDiff,
                        ...
                        if 1094 > cursor.rect.x > 980 and \
                           728 > cursor.rect.y > 684:
                            if self.curSel == 3:
                                if os.path.isfile("boards" +
                                                  os.sep + self.text + ".txt"):
                                    self.goodLoad = True
                                    if not os.path.isfile("scores" +
                                                          os.sep + self.text +
                                                          "Scores.txt"):
                                            ff = file("scores" + os.sep +
                                                      self.text +
                                                      "Scores.txt", 'w')
                                            for i in range(4):
                    ...
                    if 777 > cursor.rect.x > 466 and \
                       887 > cursor.rect.y > 765 and \
                       not self.instBool and not self.modeBool:
                        pygame.quit()

                #textbox
                if self.curSel == 3 and self.modeBool:
                    if pyevent.type == KEYDOWN and pyevent.key == K_BACKSPACE:
                        self.text = self.text[:-1]
                    elif pyevent.type == KEYDOWN:
                        self.text += pyevent.unicode.encode("ascii")
                    display_box(
                        self.menuImg, '' + self.text, 575, 380, 300, 30, 42)

                #display defaults
                if self.selSubtraction == 0 and \
                   self.modeBool and (self.curSel == 1 or self.curSel == 2):
                    self.selSub.change(self.strings['add'])
                else:
                    self.selSub.change(self.strings['sub'])

                if self.modeBool and (self.curSel == 1 or self.curSel == 2):
                    if self.curSel == 2:
                        self.selRng.change('')
                        self.selCas.change('')
                        if self.selRange == 1:
                        ...
                        if self.cas == 1:
                            ...
                            self.selCas.change(
                                self.strings['againstTime'])

                    if self.curSel == 1:
                        if self.selDiff == 1:
                            self.selDifficulty.change(
                                self.strings['sNoRotation'])
                        elif self.selDiff == 2:
                            self.selDifficulty.change(
                                self.strings['lNoRotation'])
                        elif self.selDiff == 3:
                            self.selDifficulty.change(
                                self.strings['sWithRotation'])
                        elif self.selDiff == 4:
                            self.selDifficulty.change(
                                self.strings['lWithRotation'])
                    else:
                        if self.selDiff == 1:
                            self.selDifficulty.change(
                                self.strings['sWithRotation'])
                ...
                elif self.modeBool and self.curSel == 3:
                    self.selRng.change('')
                    self.selSub.change('')
                    self.selCas.change('')
                    self.selRng2.change('')
